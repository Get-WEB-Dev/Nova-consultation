...
export default function BlogPage(){
  ...
  const doPost=async()=>{
    ...
    setPosting(true);
    try{
      const r=await fetch("/api/posts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({doctorId:user.id,...form})});
      const j=await r.json();
      if(j.data){setPosts(p=>[{...j.data,liked:false,saved:false},...p]);setCreating(false);setForm({title:"",content:"",tags:[]});}
    }catch{}
    setPosting(false);
  };
