...
import {
  Search, Send, Paperclip, MessageSquare, ChevronLeft, Calendar,
  Bell, Check, CheckCheck, Loader2, Clock, Video, BellOff,
  ChevronRight, ArrowRight, Stethoscope, Star, Phone, X, Zap,
  Users, AlertCircle, RefreshCw, RotateCcw, Activity, Heart, Shield, Eye,
} from "lucide-react";
...
type FollowUpStatus = "upcoming" | "completed" | "missed";
type DoctorPresence = "online" | "offline";
...
const RECOMMENDED_DOCTORS: RecommendedDoctor[] = [
  { id: "rec-1", name: "Dr. Sarah Chen", specialty: "General Medicine", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Sarah", presence: "online", rating: 4.9 },
  { id: "rec-2", name: "Dr. Michael Roe", specialty: "Dermatology", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Michael", presence: "online", rating: 4.8 },
  { id: "rec-3", name: "Dr. Emily Park", specialty: "Pediatrics", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Emily", presence: "offline", rating: 4.7 },
  { id: "rec-4", name: "Dr. James Obi", specialty: "Cardiology", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=James", presence: "online", rating: 4.9 },
  { id: "rec-5", name: "Dr. Ana Lopez", specialty: "Internal Medicine", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Ana", presence: "online", rating: 4.6 },
  { id: "rec-6", name: "Dr. David Kim", specialty: "Neurology", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=David", presence: "offline", rating: 4.8 },
];

const FOLLOW_UP_STATUS_CONFIG: Record<FollowUpStatus, { label: string; bg: string; text: string; dot: string }> = {
  upcoming: { label: "Upcoming", bg: "#eff6ff", text: "#1e4470", dot: "#3b82f6" },
  completed: { label: "Completed", bg: "#f0fdf4", text: "#166534", dot: "#22c55e" },
  missed: { label: "Missed", bg: "#fef2f2", text: "#991b1b", dot: "#ef4444" },
};
...
function getFollowUpStatus(scheduledAt: string, status: string): FollowUpStatus {
...
export default function ConsultationsPage() {
  ...
  const [recommendedDoctors] = useState<RecommendedDoctor[]>(RECOMMENDED_DOCTORS);
        ...
        Array.from(docMap.values()).map(async (s) => {
          try {
            ...
            return {
              ...
              lastMessageTime: last?.created_at ?? s.created_at ?? new Date().toISOString(),
              unreadCount: m.filter((x: any) => x.sender_role === "doctor" && !x.read_at).length,
              presence: isOnline ? "online" as DoctorPresence : "offline" as DoctorPresence,
              lastSeen: isOnline ? undefined : new Date(Date.now() - Math.random() * 7200000).toISOString(),
              consultationReason: s.notes ?? (s as any).symptoms ?? undefined,
            };
          } catch {
            return {
              doctorId: s.doctorId,
              ...
              unreadCount: 0,
              presence: "offline" as DoctorPresence,
            };
          }
        }),
      );
  ...
  const sendMsg = useCallback(async () => {
    ...
    setMsgs((prev) => [
      ...prev,
      {
        ...
        isPending: true, status: "sending" as MessageStatus,
      },
    ]);
    try {
      const r = await fetch("/api/messages", {
        ...
        body: JSON.stringify({
          doctorId: selectedConvo.doctorId, patientId: user.id,
          senderId: user.id, senderRole: "patient", body: text,
        }),
      });
      ...
      setMsgs((prev) =>
        prev.map((m) =>
          m.id === tempId
            ? { ...m, id: j.data?.id ?? tempId, isPending: false, status: "delivered" as MessageStatus }
            : m,
        ),
      );
      ...
      setTimeout(() => {
        setMsgs((prev) =>
          prev.map((m) =>
            m.id === (j.data?.id ?? tempId) ? { ...m, status: "seen" as MessageStatus } : m,
          ),
        );
      }, 2000);
      fetchConvos();
      ...
      setMsgs((prev) =>
        prev.map((m) =>
          m.id === tempId ? { ...m, isPending: false, status: "failed" as MessageStatus } : m,
        ),
      );
    }
  }, [msgInput, selectedConvo, user, fetchConvos]);

  const retryMsg = useCallback(async (failedMsg: Msg) => {
    if (!selectedConvo || !user) return;
    setMsgs((prev) =>
      prev.map((m) =>
        m.id === failedMsg.id ? { ...m, status: "sending" as MessageStatus, isPending: true } : m,
      ),
    );
    try {
      const r = await fetch("/api/messages", {
        ...
        body: JSON.stringify({
          doctorId: selectedConvo.doctorId, patientId: user.id,
          senderId: user.id, senderRole: "patient", body: failedMsg.text,
        }),
      });
      ...
      setMsgs((prev) =>
        prev.map((m) =>
          m.id === failedMsg.id
            ? { ...m, id: j.data?.id ?? failedMsg.id, isPending: false, status: "delivered" as MessageStatus }
            : m,
        ),
      );
      fetchConvos();
      ...
      setMsgs((prev) =>
        prev.map((m) =>
          m.id === failedMsg.id ? { ...m, isPending: false, status: "failed" as MessageStatus } : m,
        ),
      );
    }
  ...
  const handleFile = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file.size > MAX_FILE_SIZE) {
        setFileError(`File too large (${formatFileSize(file.size)}). Maximum size is 5 MB.`);
        e.target.value = "";
      ...
      setMsgs((prev) => [
        ...prev,
        {
          ...
          isPending: true, status: "sending" as MessageStatus, uploadProgress: 0,
        },
      ]);
      ...
      const progressInterval = setInterval(() => {
        setMsgs((prev) =>
          prev.map((m) =>
            m.id === tempId && m.uploadProgress !== undefined && m.uploadProgress < 90
              ? { ...m, uploadProgress: m.uploadProgress + 15 }
              : m,
          ),
        );
      }, 300);
      try {
        ...
        setMsgs((prev) => prev.map((m) => m.id === tempId ? { ...m, uploadProgress: 100 } : m));
        const mr = await fetch("/api/messages", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            doctorId: selectedConvo.doctorId, patientId: user.id,
            senderId: user.id, senderRole: "patient",
            attachmentUrl: ud.url, attachmentName: ud.name ?? file.name,
        ...
        setMsgs((prev) =>
          prev.map((m) =>
            m.id === tempId
              ? { ...m, id: mj.data?.id ?? tempId, fileUrl: ud.url, isPending: false, status: "delivered" as MessageStatus, uploadProgress: undefined }
              : m,
          ),
        );
        fetchConvos();
        ...
        clearInterval(progressInterval);
        setMsgs((prev) =>
          prev.map((m) => m.id === tempId ? { ...m, isPending: false, status: "failed" as MessageStatus, uploadProgress: undefined } : m),
        );
      } finally {
        e.target.value = "";
      }
  ...
  const TABS = [
    ...
    { key: "followups" as Tab, label: "Care Follow-ups", icon: Calendar, badge: 0 },
    {
  ...
  return (
    <div className="min-h-screen pb-10" style={{ background: "#eef2f7" }}>
          ...
          >
            <div className="flex h-full">
              ...
              >
                ...
                {/* ── Recommended Doctors ── */}
                <div className="px-3 py-3 border-b border-slate-100 flex-shrink-0" style={{ background: "#f8fafc" }}>
                  <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-2 px-1">Quick Consult</p>
                  <div className="flex gap-2.5 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
                    {recommendedDoctors.map((doc) => (
                      <button
                        key={doc.id}
                        onClick={() => router.push(`/doctors?instant=true&doctorId=${doc.id}`)}
                        className="flex flex-col items-center gap-1 min-w-[60px] group"
                      >
                        <div className="relative">
                          <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-white shadow-sm group-hover:shadow-md transition-shadow">
                            <Image src={doc.avatar} alt={doc.name} width={44} height={44} className="w-full h-full object-cover" unoptimized />
                          </div>
                          <span
                            className="absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white"
                            style={{ background: doc.presence === "online" ? "#22c55e" : "#d1d5db" }}
                          />
                        </div>
                        <span className="text-[10px] font-semibold text-slate-600 text-center truncate w-full leading-tight">
                ...
                <div className="flex-1 overflow-y-auto">
                  {loadingConvos ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-14 px-4 text-center">
                      <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-3" style={{ background: "linear-gradient(135deg, #eff6ff, #e0f2fe)" }}>
                        <Zap className="w-6 h-6" style={{ color: ACCENT }} />
                      </div>
                      <p className="font-extrabold text-slate-700 text-[14px]">Start a consultation in seconds</p>
                      <p className="text-[11px] text-slate-400 mt-1 mb-4 max-w-[200px]">Connect instantly with available doctors for quick medical advice</p>
                      <button
                        onClick={() => router.push("/doctors?instant=true")}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[12px] font-extrabold text-white transition-all active:scale-95 hover:shadow-lg"
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="relative flex-shrink-0">
                            ...
                            <span
                              className="absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white"
                              style={{ background: c.presence === "online" ? "#22c55e" : "#d1d5db" }}
                            />
                            {/* Unread dot */}
                            {c.unreadCount > 0 && (
                              <span
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between gap-1 mt-0.5">
                              <p className={`text-[11px] truncate ${c.unreadCount > 0 ? "font-medium text-slate-600" : "text-slate-400"}`}>
                                {c.presence === "online" && <span className="text-emerald-500">● </span>}
                                {c.lastMessage || (c.presence === "offline" && c.lastSeen ? `Last seen ${formatLastSeen(c.lastSeen)}` : c.doctorSpecialty)}
                              </p>
                              {c.unreadCount > 0 && (
                                <span
              ...
              {selectedConvo ? (
                <div className="flex-1 flex flex-col min-w-0">
                  ...
                  >
                    ...
                    {/* Avatar + presence */}
                    <div className="relative flex-shrink-0">
                      <div className="w-9 h-9 rounded-full overflow-hidden bg-blue-400 border-2 border-white/20">
                        <Image src={selectedConvo.doctorAvatar} alt={selectedConvo.doctorName} fill className="object-cover" unoptimized />
                      </div>
                      <span
                        className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-[#1a3558]"
                        style={{ background: selectedConvo.presence === "online" ? "#22c55e" : "#9ca3af" }}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-extrabold text-[13px] text-white truncate">{selectedConvo.doctorName}</p>
                      <p className="text-[10px] font-medium" style={{ color: "rgba(255,255,255,0.6)" }}>
                        {selectedConvo.presence === "online" ? (
                          <span className="flex items-center gap-1"><span className="text-emerald-400">●</span> Online</span>
                        ) : selectedConvo.lastSeen ? (
                          `Last seen ${formatLastSeen(selectedConvo.lastSeen)}`
                  ...
                  {selectedConvo.consultationReason && (
                    <div className="px-4 py-2.5 border-b border-slate-100 flex items-center gap-2 flex-shrink-0" style={{ background: "#fefce8" }}>
                      <Shield className="w-3.5 h-3.5 text-amber-500 flex-shrink-0" />
                      <p className="text-[11px] text-amber-700 font-medium truncate">
                        <span className="font-bold">Reason:</span> {selectedConvo.consultationReason} <span className="text-amber-400 ml-1">(Quick consultation)</span>
                      </p>
                    </div>
                  )}
                  ...
                  >
                              ...
                              >
                                ...
                                >
                                  {msg.fileType === "image" && msg.fileUrl ? (
                                    <div className="p-1.5">
                                      <img
                                        src={msg.fileUrl} alt={msg.fileName ?? "img"}
                                      ...
                                      {msg.uploadProgress !== undefined && msg.uploadProgress < 100 && (
                                        <div className="mx-2 mt-1.5 h-1 rounded-full bg-white/20 overflow-hidden">
                                          <div className="h-full rounded-full transition-all" style={{ width: `${msg.uploadProgress}%`, background: SKY }} />
                                        </div>
                                      )}
                                      {msg.text && (
                                        <p className="text-[13px] text-white px-2 pt-1 pb-1 leading-relaxed">{msg.text}</p>
                                      )}
                                      <p className={`text-[10px] px-2 pb-1.5 ${msg.from === "me" ? "text-white/50 text-right" : "text-slate-400"}`}>
                                        {msg.time}
                                      </p>
                                    </div>
                                    ...
                                    <div className="px-3.5 py-2.5">
                                      <p
                                        ...
                                        style={{ color: msg.from === "me" ? "white" : "#1e293b" }}
                                      >
                                        {msg.text}
                                      </p>
                                      <div className={`flex items-center gap-1 mt-1 ${msg.from === "me" ? "justify-end" : ""}`}>
                                        <span className="text-[10px]" style={{ color: msg.from === "me" ? "rgba(255,255,255,0.5)" : "#94a3b8" }}>
                                          {msg.time}
                                        </span>
                                        {msg.from === "me" && (() => {
                                          const s = msg.status ?? (msg.isPending ? "sending" : "delivered");
                                          if (s === "failed") return (
                                            <button onClick={() => retryMsg(msg)} className="flex items-center gap-0.5 ml-1" title="Retry">
                                              <AlertCircle className="w-3 h-3 text-red-400" />
                                            </button>
                                          );
                                          if (s === "sending") return <Loader2 className="w-3 h-3 text-white/50 animate-spin" />;
                                          if (s === "sent") return <Check className="w-3 h-3 text-white/50" />;
                                          if (s === "delivered") return <CheckCheck className="w-3 h-3 text-white/60" />;
                                          if (s === "seen") return <CheckCheck className="w-3 h-3" style={{ color: SKY }} />;
                                          return null;
                                        })()}
                                      </div>
                                      {msg.status === "failed" && (
                                        <button
                                          onClick={() => retryMsg(msg)}
                                          className="flex items-center gap-1 mt-1.5 text-[10px] font-semibold text-red-300 hover:text-red-200 transition-colors"
                                        >
                    ...
                    {/* File error toast */}
                    {fileError && (
                      <div className="flex items-center gap-2 p-3 rounded-xl border border-red-200 bg-red-50 animate-fade-in">
                        <AlertCircle className="w-4 h-4 text-red-500 flex-shrink-0" />
                        <p className="text-[12px] text-red-600 font-medium">{fileError}</p>
                      </div>
                    )}
            ...
            {selectedFU ? (
              ...
              <>
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="font-extrabold text-slate-900 text-[16px]">Care Follow-ups</h2>
                    <p className="text-[11px] text-slate-400 mt-0.5">{followUps.length} scheduled</p>
                  </div>
                </div>
                    ...
                    {followUps.map((fu) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <p className="font-extrabold text-[13px] text-slate-900">{fu.doctorName}</p>
                              {(() => {
                                const fStatus = getFollowUpStatus(fu.scheduledAt, fu.status);
                                const cfg = FOLLOW_UP_STATUS_CONFIG[fStatus];
                                return (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold" style={{ background: cfg.bg, color: cfg.text }}>
                            ...
                            <p className="text-[11px] font-semibold mt-0.5" style={{ color: ACCENT }}>{fu.doctorSpecialty}</p>
                            <div className="flex items-center gap-1.5 mt-1.5 text-[11px] text-slate-500">
                              <Clock className="w-3 h-3 flex-shrink-0" style={{ color: ACCENT }} />
                              <span>{formatDate(fu.scheduledAt)}</span>
                            </div>
                            <p className="text-[11px] text-slate-400 mt-1 truncate">{fu.reason}</p>
                          </div>
      ...
      <button
        onClick={() => router.push("/doctors?instant=true")}
        className="fixed bottom-24 md:bottom-8 right-5 md:right-8 w-14 h-14 rounded-full flex items-center justify-center text-white shadow-xl hover:shadow-2xl transition-all hover:scale-110 active:scale-95 z-50"
        style={{ background: `linear-gradient(135deg, ${NAV_BG}, ${SKY})` }}
        title="Start instant consultation"
      >
